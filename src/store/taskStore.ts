...
async function cleanupLocalCalendarDuplicates(tasks: Task[]): Promise<Task[]> {
  ...
  for (const task of tasks) {
    // Series anchors are never considered duplicates — keep them safe
    // from the dedup pass even if they share a googleCalendarEventId
    // with a one-off generated by GCal recurrence overrides.
  ...
  for (const task of tasks) {
    ...
    // Don't delete a series anchor just because another series happens
    // to cover its date — that would erase the user's recurring task.
    if (task.recurrenceRule) continue;
