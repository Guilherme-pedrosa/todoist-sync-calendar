...
async function cleanupLocalCalendarDuplicates(tasks: Task[]): Promise<Task[]> {
  // ⚠️ DESATIVADO: esta função estava deletando tarefas legítimas no auto-sync.
  // Cenários que causavam perda de dados:
  //   1. Duas tarefas com o mesmo google_calendar_event_id (acontece quando
  //      o backfill roda 2x ou quando o GCal devolve override de recorrência) →
  //      deletava uma das duas, mesmo sendo conteúdos diferentes.
  //   2. Qualquer série recorrente que "cobrisse" o dia de uma tarefa pontual
  //      com título parecido → deletava a pontual (ex.: série "Almoço" deletava
  //      "Reunião Jonas" se títulos batessem em algum dia).
  // O cleanup agora só roda manualmente via botão de "Limpar duplicatas" nas
  // configurações, NUNCA no fetchData automático.
  return tasks;
}
