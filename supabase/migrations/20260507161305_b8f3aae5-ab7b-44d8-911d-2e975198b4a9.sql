...
BEGIN
  ...
  LOOP
    ...
    session_clamped AS (
      SELECT
        ...
        s.active_seconds, s.idle_seconds,
        s.started_at AS raw_start,
        COALESCE(s.ended_at, s.last_seen_at) AS raw_end
      FROM public.activity_sessions s
      WHERE s.started_at <= v_day_end
        AND COALESCE(s.ended_at, s.last_seen_at) >= v_day_start
    ),
    session_valid AS (
      SELECT * FROM session_clamped WHERE hi > lo
    ),
    session_marked AS (
      SELECT
        user_id, workspace_id, lo, hi, active_seconds, idle_seconds, raw_start, raw_end,
        CASE
          WHEN lo <= MAX(hi) OVER (
    ...
    session_active_raw AS (
      SELECT
        user_id, workspace_id,
        SUM(
          (active_seconds * EXTRACT(EPOCH FROM (hi - lo)) /
           NULLIF(EXTRACT(EPOCH FROM (raw_end - raw_start)), 0))
        )::int AS active_raw,
        SUM(
    ...
    session_final AS (
      SELECT
        so.user_id, so.workspace_id,
        so.online_seconds, so.first_seen_at, so.last_seen_at, so.sessions_count,
        LEAST(COALESCE(sar.active_raw, 0), so.online_seconds) AS active_seconds,
        LEAST(
          COALESCE(sar.idle_raw, 0),
          GREATEST(so.online_seconds - LEAST(COALESCE(sar.active_raw, 0), so.online_seconds), 0)
        ) AS idle_seconds
      FROM session_online so
      LEFT JOIN session_active_raw sar
        ON sar.user_id = so.user_id AND sar.workspace_id = so.workspace_id
    ),
    hourly AS (
    ...
    hourly_json AS (
      ...
      FROM hourly GROUP BY user_id, workspace_id
    ),
    completions AS (
      SELECT al.user_id, t.workspace_id, t.project_id, p.name AS project_name, p.is_inbox,
        COUNT(*)::int AS qtd
      FROM public.activity_log al
      JOIN public.tasks t ON t.id = COALESCE((al.payload->>'task_id')::uuid, al.entity_id)
      LEFT JOIN public.projects p ON p.id = t.project_id
      WHERE al.entity_type = 'task' AND al.action = 'completed'
        AND al.created_at >= v_day_start AND al.created_at <= v_day_end
    ...
    visits_raw AS (
      SELECT
        v.user_id, v.workspace_id, v.domain,
    ...
    visits_agg AS (
      SELECT
        user_id, workspace_id,
        (SUM(EXTRACT(EPOCH FROM (hi - lo))) FILTER (WHERE category = 'productive'))::int AS productive_seconds,
        (SUM(EXTRACT(EPOCH FROM (hi - lo))) FILTER (WHERE category = 'neutral'))::int AS neutral_seconds,
        (SUM(EXTRACT(EPOCH FROM (hi - lo))) FILTER (WHERE category = 'distracting'))::int AS distracting_seconds
      FROM visits_merged
      GROUP BY user_id, workspace_id
    ),
    visits_per_domain AS (
      SELECT
        user_id, workspace_id, domain, category,
    ...
    final AS (
      SELECT
        sf.user_id, sf.workspace_id, v_day AS day,
        sf.active_seconds, sf.idle_seconds, sf.online_seconds, sf.sessions_count,
        sf.first_seen_at, sf.last_seen_at,
      ...
      FROM session_final sf
      LEFT JOIN completions_agg ca ON ca.user_id = sf.user_id AND ca.workspace_id = sf.workspace_id
      LEFT JOIN visits_agg va ON va.user_id = sf.user_id AND va.workspace_id = sf.workspace_id
      LEFT JOIN top_domains_agg td ON td.user_id = sf.user_id AND td.workspace_id = sf.workspace_id
      LEFT JOIN hourly_json hj ON hj.user_id = sf.user_id AND hj.workspace_id = sf.workspace_id
    ),
    final_capped AS (
      SELECT
        f.*,
    ...
    SELECT
      ...
      LEAST(100, GREATEST(0,
        (50.0 * f.active_seconds / NULLIF(f.online_seconds, 0)) +
        (30.0 * LEAST(f.tasks_completed, 10) / 10.0) +
        (20.0 * f.prod_raw / NULLIF(f.prod_raw + f.dist_raw, 0))
      ))::int,
  ...
  RETURN jsonb_build_object('ok', true, 'days', v_days, 'processed', v_processed, 'computed_at', now());
END $function$;